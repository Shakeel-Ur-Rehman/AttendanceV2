...
export class AttendanceService {
  ...
  async create(data: any, @Request() req) {
    ...
    AttendanceHelpers.matchLocation(
      data,
      await this.locationService.findByIds(data.locationId),
    );
  ...
  async checkIn(employee, todaysAttendances, data, location) {
    ...
    if (process.env.IS_SYNC && todaysAttendances.length == 0) {
      res = await this.httpHelperService.createCheckIn({
        username: employee.authUser.username,
        location,
        checkInTime,
      });
