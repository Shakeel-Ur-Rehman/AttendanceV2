...
export class AuthService {
    ...
    async getLoggedUserDetails(user){
      ...
      if (
        ...
        //me = await this.adminService.findByAuthUserId(user.id);
    ...
    async registerDevice(user,data){
        if (user.device_id == null) {
            try {
              await this.usersService.update(user.id,{device_id:data.deviceId,device_type: data.deviceType});
            } catch (error) {
              const user = await this.usersService.findOneByCondition({device_id: data.device_id});
    ...
    async checkHRMDetails(user){
        if (user.type === UserRolesEnum.EMPLOYEE && process.env.IS_SYNC) {
            ...
            if(!res){
                throw new HttpException({message: `Invalid Batch number.`},HttpStatus.NOT_FOUND);
            }
